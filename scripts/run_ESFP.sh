    ...
    for sampler in "${poled_samplers[@]}"; do
      ...
      for run in "${poled_runs[@]}"; do
        ...
        for prob_init in "${poled_prob_inits[@]}"; do
          ...
          docker exec -it "$method_docker_id" \
            env PYTHONPATH="$method_docker_app":"$PYTHONPATH" \
            python "$method_train" --dataset realevents --dataroot "$data_docker_path" --pretrained "$method_docker_app"/results/original --netinput events_8_bins_cvgri --batch_size 4 \
                                   --logs_oled "$logs_docker_res" --training_mode test
